...
{
    // std::vector<Vertex> _m(sizeX * sizeY);
    // std::vector<unsigned int> _id(((sizeX - 1) + (sizeY - 1)) * 6);
    for (int i = 0; i < sizeX; i++)
    {
        for (int c = 0; c < sizeY; c++) {
            Vertex v;
            //((float)i*(float)c*40.0f)/100.0f
            v.Position = topLeft+glm::vec3(i * gridWidth, -c * gridWidth, 0.0f);
            v.PrevPos = v.Position;
            v.Normal = glm::vec3(0.0f, 0.0f, -1.0f);
            
            //Lock all top for now
            // if (c == 0 || c == 1)
            //     v.locked = true;
            // else
            //     v.locked = false;
...
void simulate (Mesh* mesh)
{
    for (int i = 0; i < mesh->vertices.size(); i++)
    {
        ...
        mesh->vertices[i].Position += glm::vec3(0.0f, -0.03f, glm::sin(mesh->vertices[i].Position.x*glfwGetTime()*0.3f)*0.005f);
